...
| B7 | §11 Risk 1 — "cache scores for identical feature hashes (1-min TTL)" | No feature-hash score cache exists | ✅ **Fixed, then corrected** — `score_cache.py`, wired into `scorer.py` + `api.py`. **A critical defect shipped with the original fix and was found later, live, on 2026-09-03**: `feature_hash()` hashed `list(x_row)` where `x_row` is a pandas DataFrame — `list()` on a DataFrame yields its column names, not the row's values, so every transaction hashed to the same key. Once any transaction was scored, every other transaction scored within the next 60s silently received *that* transaction's score and decision, regardless of its own risk. The original "live-verified via identical `fraud_score` on repeat calls" check could not have caught this — repeating the *same* event looks correct whether the cache is keyed correctly or not; it takes two *different* events to expose it. Found via a flaky e2e test, root-caused by direct reproduction (a $4.25 US event and a $999,999 Nigeria event hashed identically), fixed to hash actual values, and confirmed by running the full suite twice back-to-back inside one cache TTL window (87/87 both times). See `section_10_2_target_audit.md` for the general lesson this leaves about single-event live-verification. |
